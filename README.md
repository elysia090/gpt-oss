...
The Sera reference implementation can also be driven from the lightweight transfer manifest generated by `python -m gpt_oss.tools.sera_transfer`. Once you have produced the artefacts you can chat with the model using:

```bash
python -m gpt_oss.cli.sera_chat --manifest /path/to/manifest --tool python --prompt "hello"
```

The manifest directory must contain `sera_manifest.bin` and a runtime snapshot such as `sera_state.pkl`. Tools are optional and can be enabled with repeated `--tool` flags (currently `browser` and `python`).
