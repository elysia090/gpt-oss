...
#       See gpt_oss.cli.chat_cli for a more complete example with the Harmony parser.
# torchrun --nproc-per-node=4 -m gpt_oss.cli.generate_cli -p "why did the chicken cross the road?" model/
...
def main(args):
    match args.backend:
        case "torch":
            from gpt_oss.inference.torch.utils import init_distributed
            from gpt_oss.inference.torch.model import TokenGenerator as TorchGenerator
            device = init_distributed()
            generator = TorchGenerator(args.checkpoint, device=device)
        case "triton":
            from gpt_oss.inference.torch.utils import init_distributed
            from gpt_oss.inference.triton.model import TokenGenerator as TritonGenerator
            device = init_distributed()
            generator = TritonGenerator(args.checkpoint, context=args.context_length, device=device)
        case "vllm":
            from gpt_oss.inference.vllm.token_generator import TokenGenerator as VLLMGenerator
