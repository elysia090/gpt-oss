"""Compatibility package redirecting to :mod:`gpt_oss.api.responses`.

This module keeps the historic :mod:`gpt_oss.responses_api` import path
available while all implementations now live under
:mod:`gpt_oss.api.responses`.  Only the package layout changed; the public
API stays untouched.  To avoid maintaining a tree of thin pass-through
modules we dynamically alias the new modules at import time.
"""
...
# Ensure the package itself is available in ``sys.modules`` before creating
# aliases so parents can be resolved.
sys.modules.setdefault(__name__, sys.modules[__name__])

# Initialise the compatibility aliases once the package is imported.
_prime_aliases(_SUBMODULES)

# Re-export the public API of the new package at the package level so that
# ``from gpt_oss.responses_api import create_api_server`` keeps working.
...
def __getattr__(name: str):
    """Delegate attribute access to :mod:`gpt_oss.api.responses`."""
